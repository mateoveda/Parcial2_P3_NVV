...
//AGREGUÉ ESTA FUNCION PORQUE NO ESTABA Y NO ME PERMITIA COMPILAR
NodoProducto *ArbolProducto::findMaxAndRemove(NodoProducto *r, bool *found) {
    // Implementación de la lógica para encontrar el valor máximo en el árbol
    // y eliminarlo. Esto podría requerir un recorrido específico en el árbol.

    // Pseudocódigo para encontrar y eliminar el nodo con el valor máximo.
    if (r->getRight() == nullptr) {
        *found = true;
        return r; // Nodo con el valor máximo
    }

    NodoProducto *result = findMaxAndRemove(r->getRight(), found);
    if (*found) {
        // Eliminar el nodo con el valor máximo
